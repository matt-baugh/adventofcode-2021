...
convert :: String -> [[Int]]
convert xs
  = concatMap (\(rs, a) -> map (map (wrapAdd a)) rs) $ zip (repeat rRep) [0..4]
  where
    orig = map (map (\c -> ord c - ord '0')) $ lines xs
    rRep = map (\r -> concatMap (\(r', a) -> map (wrapAdd a) r') $ zip (repeat r) [0..4]) orig
    wrapAdd :: Int -> Int -> Int
    wrapAdd a b
      | s > 9     = s - 9
      | otherwise = s
